    ...
    while(T--) {
        ...
        for(int i=0; i<n; i++) {
            if(s[i] == '0') {
                ...
                // if(prev == '1' || next == '1') { // todo: this block is causing trouble
                //     w[i] = '1';
                // }
            }
        }
        // there is no impossible case, looks like rechacking w to generate x is the only way
        bool b = true;
        for(int i=0; i<n; i++) {
            char prev = 0, next = 0, curr = '0';
            if(i-x >=0) {    prev = w[i-x];  }
            if(i+x < n) {    next = w[i+x];  }
            if(prev == '1' || next == '1') {
                curr = '1';
