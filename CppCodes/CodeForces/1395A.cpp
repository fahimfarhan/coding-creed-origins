...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    while(T--) {
        ...
        cin>>kolors[0]>>kolors[1]>>kolors[2]>>kolors[3];
        
        oddParityKount = 0;
        for(int i=0; i<4; i++) {
            if( (kolors[i] & 1) == 1) {    oddParityKount++; }
        }
        if(oddParityKount > 1) {
            minimus = min(kolors[0], kolors[1]);
            minimus = min(kolors[2], minimus);
            minimus = min((ll)1, minimus);

            if(minimus >=1) {
                for(int i=0; i<3; i++) {    kolors[i]--; }
                kolors[3]+=3;
                oddParityKount = 0;
                for(int i=0; i<4; i++) {
                    if( (kolors[i] & 1) == 1) {    oddParityKount++; }
