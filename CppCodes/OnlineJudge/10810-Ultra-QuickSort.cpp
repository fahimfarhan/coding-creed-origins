...
ll conquer(ll *a, int l, int r) {
    ll swapKount = 0;
    int size = r - l + 1;
    ll *sortedArray;
    sortedArray = new ll[size];
    ...
    while(i1<=mid) {
        sortedArray[k1] = a[i1]; i1++; k1++; 
    }

    while(j1<=r) {
        sortedArray[k1] = a[j1]; j1++; k1++;
    }
...
ll divide(ll *array, int l, int r) {
    if(r<l) {   return 0; }
    if(r-l+1 == 1) {    return 0; }
    int mid = (l+r) / 2;
    ll kount1 = divide(array,  l, mid);
    ll kount2 = divide(array, mid+1, r);
    ll kount3 = conquer(array, l, r);
    ll kount = kount1 + kount2 + kount3;
    return kount;
}

ll ultraquicksort(ll *a, int l, int r) {
    return divide(a, l, r);
}
