...
int main(int argc, char const *argv[]){
  ...
  ll T, n, m, kount, **a;
  ll w,x,y,z, sum, avg, remainder, minimus;
  cin>>T;
  while(T--) {
    cin>>n>>m;
    a = new ll*[n+1];
    ...
    for(int i=0; i<n/2; i++) {
      for(int j=0; j<m/2; j++) {
        ...
        remainder = sum - ((sum>>2)<<2); // to remove costly % operation
        minimus = min(remainder, 4 - remainder);
        avg+=minimus;

        kount = abs(avg-w)+abs(avg-x)+abs(avg-y)+abs(avg-z);
      }
    }
