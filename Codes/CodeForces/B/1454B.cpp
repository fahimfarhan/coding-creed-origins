...
int main(int argc, char const *argv[]){
  ...
  while(T--) {
    ...
    ll minimus = INT_MAX;
    int minimusIndex = -1;
    // bool isAtLeastOneUnique = false;
    for(auto entry: mp) {
      if(entry.second.first == 1) {
        ll currentNumber = entry.first;
        if(currentNumber < minimus) {
          minimus = currentNumber;
          minimusIndex = entry.second.second;
        }
        // minimus = min(minimus, entry.first);
        // isAtLeastOneUnique = true;
      }
    }
    // if(!isAtLeastOneUnique) {    minimus = -1;   }
